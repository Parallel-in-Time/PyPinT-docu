    ...
    \item use of Python's full feature set
      \begin{itemize}
        \item easy parallelism\\
          {\scriptsize via Python's own (e.g. \texttt{multiprocessing}) or 3rd-party modules (e.g. \texttt{mpi4py})}
	\item built-in portability\\
	  {\scriptsize runs on Unix and MacOS (should run on Windows too)}
      \end{itemize}
  \end{itemize}
\end{frame}
