...
\begin{frame}
  ...
  \framesubtitle{
    \scriptsize\normalfont
    \alt<1>{V. Pereyra, “On improving an approximate solution of a functional equation by deferred corrections,” Numer. Math., vol. 391, no. 044, pp. 376–391, 1966.}
...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{5cm}
      ...
      \hspace{0.75em}\texttt{\textasciitilde.integratos}\\
  ...
  \begin{columns}[T]
    ...
    \begin{column}{10cm}
      \begin{itemize}
        ...
        \item providers aggregated in basic quadrature interface\\
          {\scriptsize for intuitive methods:\\
           \texttt{.q\_matrix.apply(data)} and \texttt{.s\_matrix.apply(data, target\_node)} \\}
      \end{itemize}
      
      \begin{picture}(1,1)
        \put(25,-100){\includegraphics[height=3.75cm]{src/quadrature_interfaces.pdf}}
      \end{picture}
    \end{column}
  \end{columns}
\end{frame}
